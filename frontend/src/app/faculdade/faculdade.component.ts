...
import { FaculdadeService, FaculdadeEntity } from '../_services/faculdade.service';
...
import { CursoEntity, CursoService } from '../_services/curso.service';
...
export class FaculdadeComponent implements OnInit {
  ...
  public displayedColumns: string[] = ['id', 'nome', 'curso', 'options'];
  public faculdades: FaculdadeEntity[] = [];
  public cursos: CursoEntity[] = [];

  public dataSource = new MatTableDataSource<FaculdadeEntity>();

  public errorMessage: string;
  public loading: boolean;

  public faculdade: FaculdadeEntity = new FaculdadeEntity();

  @ViewChild(MatSidenav, {static: true}) sidenav: MatSidenav;

  constructor(
    private service: FaculdadeService,
  ...
  ngOnInit(): void {
    ...
    //Carrega a lista de faculdades
    this.service.listarTodos().subscribe(result => {
      
      //Alimenta o datasource da tabela com a lista recebido da service
      this.faculdades = result as [];

      //Alimenta o datasource com os faculdades
      this.dataSource.data = this.faculdades;

      // alimenta o datasource com usuarios
      this.cursoService.listarTodos().subscribe(result => {
    ...
    //Listner do evento createFaculdade
    this.socketClient.fromEvent('createFaculdade').subscribe(result => {
      this.faculdades.push(result as FaculdadeEntity)
      this.dataSource.data = this.faculdades;
    })

    //Listner do evento deleteFaculdade
    this.socketClient.fromEvent('deleteFaculdade').subscribe(result => {
      let faculdade = result as FaculdadeEntity;
      let index = this.faculdades.findIndex( item => item.id == faculdade.id);

      this.faculdades.splice(index, 1);

      this.dataSource.data = this.faculdades;
    })

    //Listner do evento createFaculdade
    this.socketClient.fromEvent('updateFaculdade').subscribe(result => {
      let faculdade = result as FaculdadeEntity;
      let index = this.faculdades.findIndex( item => item.id == faculdade.id);

      this.faculdades[index] = faculdade;
  ...
  public confirmar(): void {
    ...
    this.service.salvar(this.faculdade).subscribe(result => {
   ...
   * @param faculdade 
   */
  public excluir(faculdade: FaculdadeEntity): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.service.excluir(faculdade.id).subscribe(result => {
  ...
  public adicionar(): void {
    ...
    this.openSidenav(new FaculdadeEntity());
  }
   ...
   * @param faculdade
   */
  public editar(faculdade: FaculdadeEntity): void {
    //Como faculdade é passado um objeto da tabela por referencia, 
    //se não foir feito uma copia deste, ao alterar a linha da 
    //tabela altera junto.
    this.openSidenav(Object.create(faculdade));
  }
   ...
   * @param faculdade 
   */
  private openSidenav(faculdade: FaculdadeEntity): void {
